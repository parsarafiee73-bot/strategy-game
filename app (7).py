...
# ============================================================
# GAME ENGINE (v3: military, tech tiers, treaties, events, country perks)
# ============================================================
...
def action_cost(action: str, p: Dict[str, Any]) -> int:
    ...
    if action == "research":
        ...
        return int(base * (100 - perk_value(p.get("country"), "research_discount_pct")) / 100)
    if action == "recruit":
        return 250
    if action == "welfare":
        return 150
...
def _save_player(cur, p: Dict[str, Any]):
    ...
    p["military"] = max(0, int(p.get("military") or 0))
    ...
    cur.execute(
        ...
        (
            ...
            p["population"], p["military"], p["actions_left"], p["score"], p["id"],
        ),
    )
...
def _lock_turn(cur, game_id: int, user_id: int):
    ...
    if current.get("military") is None:
        current["military"] = 0
    return game, current
...
def _treaty_between(cur, game_id: int, a: int, b: int, rnd: int) -> bool:
    ...
    cur.execute(
        ...
        WHERE game_id = %s AND player_a = %s AND player_b = %s AND expires_round >= %s
        """,
        (game_id, lo, hi, rnd),
    )
    return cur.fetchone() is not None
...
def _treaty_count(cur, game_id: int, player_id: int, rnd: int) -> int:
    cur.execute(
        ...
        WHERE game_id = %s AND expires_round >= %s AND (player_a = %s OR player_b = %s)
        """,
...
def resolve_battle(attacker: Dict[str, Any], defender: Dict[str, Any], rng=random) -> Dict[str, Any]:
    ...
    atk = attacker["military"] * (1 + 0.08 * tier) * rng.uniform(0.85, 1.15)
    dfn = (
        (defender["military"] * 1.15 + defender["industry"] * 0.2)
        * (1 + perk_value(defender.get("country"), "defense_pct") / 100)
...
def settle_turn_end(current: Dict[str, Any], rng=random) -> Dict[str, Any]:
...
def action_for_player(game_id: int, user_id: int, action: str):
    ...
    if action not in ("build", "research", "trade", "recruit", "welfare"):
        raise ValueError("unknown_action")
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            if action == "build":
                gain = 2 + perk_value(country, "build_bonus")
                p["industry"] += gain
                p["population"] += 1
                info["n"] = gain
            elif action == "research":
                gain = 3 + perk_value(country, "research_bonus")
                p["science"] += gain
                info["n"] = gain
            elif action == "trade":
                ...
                gain = int(base * market * (1 + perk_value(country, "trade_pct") / 100))
                p["money"] += gain
                p["stability"] -= 1
                info["gain"] = gain
                ...
                gain = 3 + perk_value(country, "recruit_bonus")
                p["military"] += gain
                info["n"] = gain
            elif action == "welfare":
                p["stability"] += 8
...
def attack_player(game_id: int, user_id: int, target_id: int):
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            d = cur.fetchone()
            if not d or d["id"] == p["id"]:
                raise ValueError("bad_target")
            if d.get("military") is None:
                d["military"] = 0
...
def sign_treaty(game_id: int, user_id: int, target_id: int):
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            until = rnd + 2
            cur.execute(
                "INSERT INTO treaties (game_id, player_a, player_b, expires_round) VALUES (%s, %s, %s, %s)",
                (game_id, min(p["id"], d["id"]), max(p["id"], d["id"]), until),
            )

            p["money"] += 60 - cost
            p["stability"] += 3 + perk_value(p.get("country"), "diplo_stab")
            p["actions_left"] -= 1
            d["money"] += 60
            d["stability"] += 3
            _save_player(cur, p)
...
def end_turn(game_id: int, user_id: int) -> Dict[str, Any]:
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            report = settle_turn_end(current)
            ...
            if total_turn > MAX_ROUNDS * n:
                cur.execute(
                    ...
                    SET score = money + (industry * 50) + (science * 100)
                        + (stability * 10) + (population * 5) + (COALESCE(military, 0) * 40)
                    WHERE game_id = %s
                    """,
                    (game_id,),
                )
                ...
                return {"finished": True, "turn": total_turn, "results": list(cur.fetchall()), **report}
            ...
            return {
                "finished": False,
                "turn": total_turn,
                "round": current_round(total_turn, n),
...
def format_game(game_id: int, language: str) -> str:
    ...
    if status_key == "running":
        ...
        if treaties:
            ...
            for tr in treaties:
                a, b = by_id.get(tr["player_a"]), by_id.get(tr["player_b"])
                if a and b:
                    text.append(f"{pname(a)} ↔ {pname(b)} ({T['until_round']} {tr['expires_round']})")
    else:
...
def format_action_result(info: Dict[str, Any], language: str) -> str:
    action = info["action"]
    key = "res_attack_win" if action == "attack" and info.get("win") else (
        "res_attack_lose" if action == "attack" else f"res_{action}"
    )
    return TEXTS[language][key].format(**info)
...
def handle_callback(callback: Dict[str, Any]):
    ...
    try:
        ...
        if data.startswith("action:"):
            ...
            game_id = int(game_id_str)
            try:
                if action in ("attack", "diplomacy"):
                    targets = get_targets(game_id, user_id)
                    answer_callback(callback_id)
                    send_message(
                        chat_id,
                        t(user_id, "pick_target_attack" if action == "attack" else "pick_target_diplomacy"),
        ...
        if data.startswith("dotarget:"):
            ...
            try:
                if action == "attack":
                    player, defender, info = attack_player(game_id, user_id, int(target_str))
                elif action == "diplomacy":
                    player, defender, info = sign_treaty(game_id, user_id, int(target_str))
                else:
                    answer_callback(callback_id)
                    return
            except ValueError as exc:
